...
(async () => {
  ...
  assert.ok(/href="\/privacy"/.test(html), "home footer/nav links to /privacy");
  console.log("ok - GET /privacy is 200 with a policy, not a dashboard redirect");

  /* 4c) Structural sections the competitor page has, rebuilt as Erasezo.
        Anchors rather than copy — headlines are allowed to change. */
  assert.ok(/id="compare"/.test(html), "comparison section is present");
  assert.ok(/Generic cloud removers/.test(html) && /AI inpainting/.test(html),
    "comparison is vs categories, not a named competitor");
  assert.ok(!/erasio\.io/i.test(html + js), "home does not point at erasio.io");
  assert.ok(/id="scope"/.test(html), "ethics / scope section is present");
  assert.ok(/SynthID/.test(html), "scope says SynthID is left alone");
  assert.ok(/id="benefits"/.test(html) && /id="features"/.test(html) && /id="how"/.test(html),
    "benefits, features and how-it-works sections exist");
  assert.ok(/id="live-demo"/.test(html) && /id="wipe"/.test(html), "live demo keeps the drag-compare");
  assert.ok(/id="dock"/.test(html), "floating install dock exists");
  assert.ok(/id="announce"/.test(html), "announcement bar exists");
  assert.ok(/Chrome listing on the way|Chrome Web Store listing is on the way/.test(html),
    "announce is about Erasezo's listing, not an install of someone else");
  assert.ok(!/id="reviews"[\s\S]{0,800}[“"]I['’]ve/.test(html),
    "reviews section does not invent a testimonial");
  assert.ok(/Reviews when the Store listing is live/.test(html),
    "reviews are deferred until a real listing exists");
  assert.ok(!/no sign-?up required/i.test(html), "does not claim sign-in is optional");
  assert.ok(!/play\.google\.com|Google Play|badge.*android/i.test(html),
    "no Play Store badge for an app that does not exist");
  assert.ok(/Product/.test(html) && /Resources/.test(html) && /Legal/.test(html),
    "footer has Product / Resources / Legal");
  console.log("ok - comparison, ethics, honesty anchors are in place");
